"""
Pytest + Selenium conftest: config fixture, logging, Chrome driver, screenshot on failure.
"""
import json
import logging
import os
...
@pytest.fixture()
def driver(config):
    """Creates Chrome from config; quits after each test."""
    options = Options()
...
# ---------- screenshot on failure (pytest-html / Allure) ----------


@pytest.hookimpl(hookwrapper=True)
def pytest_runtest_makereport(item, call):
    """
    After the test *call* phase, if it failed and the test used `driver`,
    save a screenshot and attach to pytest-html / Allure when installed.
    """
    outcome = yield
    ...
    ss_path = ss_dir / f"{safe_name}_{timestamp}.png"

    try:
        driver.save_screenshot(str(ss_path))
        logging.getLogger(__name__).info("Saved screenshot: %s", ss_path)
    except Exception:
        logging.getLogger(__name__).exception("Failed to save screenshot")
    ...
    if item.config.pluginmanager.hasplugin("allure_pytest"):
        try:
            ...
            logging.getLogger(__name__).warning("Allure attach failed: %s", e)
